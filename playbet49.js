  ...
  async function get_draw_details(){
    ...
    async function getBetZeros(presentDraw, lastDraw, _id){
      ...
      // console.log(presentDraw,lastDraw);
       ...
       won = presentDraw.filter(element => dragon.includes(element)).length > 0;
       wonStake = staked && won;
       ...
       // dragon = await getFourBalls();
       for(i in eligibleDraws)eligibleDraws[i] = await eligibleDraws[i].ball;
       console.log(_id, '-> 20last',twentyMore,' | ', colorIt(dragon[0]),colorIt(dragon[1]),colorIt(dragon[2]),colorIt(dragon[3]),eligibleDraws, won?'Hit :-)':'Missed :-(',intersection,'<- appeared');
       // console.log(allBalls);
       staked = false;
       if(stakeCount>=0 || dragon.length>14 || startStaking){
    ...
    async function stakeOnDragon(stake){
      ...
      await page.waitFor(1000);
      await page.$$eval('.game-nav__item', elem => elem[0].click()).catch(err => console.error(err.message));
