  ...
  async function get_draw_details(){
    ...
    async function getBetZeros(presentDraw, lastDraw, _id){
      ...
      // console.log(presentDraw,lastDraw);
       ...
       console.log(_id, '-> last result',highlowValue,' | ', highlow, won?'Hit :-)':'Missed :-(');
       staked = false;

       if(presentDraw.length > 0){
          console.log('high low is :::> '+highlowValue);
          highStakesReset = highlowValue === 'high' ? 0 : highStakesReset + 1;
          lowStakesReset = highlowValue === 'low' ? 0 : lowStakesReset + 1;
       }

       const highStakeAmount = getStakeAmount(highStakesReset).toString();
       const lowStakeAmount = getStakeAmount(lowStakesReset).toString();
       await stakeOnHighlow(0, highStakeAmount); // args; high, stakeAmount
       await stakeOnHighlow(2, lowStakeAmount); // args; low, stakeAmount
    ...
    function getStakeAmount(reset){
      switch (/*stakeReset*/ reset) {
          //2lives => .25, .75 -> N250 |Doubles in 7draws
          //3lives => .09091, .27273, .63636 -> N750 |Doubles in 16draws
          //4lives => .03846, .11538, .26923, .57692 -> N2,050 |Doubles in 48draws
          //5lives => .01754, .05263, .12281, .26316, .54386 -> N5,500 |Doubles in 72draws

        case 0: return Math.floor(highestAmount*.0175);
        case 1: return Math.floor(highestAmount*.0526);
        case 2: return Math.floor(highestAmount*.1228);
        case 3: return Math.floor(highestAmount*.2632);
        case 4: return Math.floor(highestAmount*.5439);
        default: console.log("\007");
                ...
                highStakesReset = 0;
                lowStakesReset = 0;
                return Math.floor(highestAmount*.0175);
      }
    }
    async function stakeOnHighlow(stake, stakeAmountValue){
      const stakeOnHigh = 0;
      const stakeOnLow = 2;
      ...
      stakeAmount = stakeAmountValue? stakeAmountValue : await getStakeAmount().toString();
